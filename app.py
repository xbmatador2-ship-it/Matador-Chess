...
def analyze_move_transition(
    ...
    if best_reply_san and best_reply_san != "-":

        parts.append(
            f"La meilleure réponse adverse indiquée par le moteur est "
            f"{best_reply_san}."
        )
...
def explain_move_problem(
    ...
    """
    Explique pourquoi LE COUP JOUÉ pose problème.

    L'ordre de raisonnement est volontairement :

        POSITION AVANT
        → menace adverse
        → COUP JOUÉ
        → changement créé par le coup
        → POSITION APRÈS
        → réponse adverse
        → conséquence

    Une caractéristique de la position avant le coup n'est donc
    jamais utilisée seule pour conclure que le coup est mauvais.
    """
    ...
    # --------------------------------------------------------
    # COUP SANS PROBLÈME SIGNIFICATIF
    # --------------------------------------------------------
    ...
    if castling_context:
        ...
        if castling_context["assessment"] == "cohérent":
            ...
            return (
                ...
                "Il faut juger le coup sur la sécurité réelle du roi et "
                "sur l'activité de la tour, pas simplement sur le fait "
                "qu'une ligne s'ouvre. Ici, la position après le roque "
                "reste suffisamment sûre."
            )

    # --------------------------------------------------------
    # COUP QUI CRÉE UNE NOUVELLE VULNÉRABILITÉ
    # --------------------------------------------------------

    new_hanging = [
        item
        for item in transition["new_user_vulnerabilities"]
    ...
    if new_hanging:
        ...
        if best_reply_san and best_reply_san != "-":
            ...
            reply_text = (
                f" La réponse adverse la plus forte est "
                f"{best_reply_san}."
            )
        ...
        return (
            ...
            f"nouvelle faiblesse.{reply_text}"
        )

    # --------------------------------------------------------
    # COUP QUI LAISSE UNE PIÈCE SOUS PRESSION
    # --------------------------------------------------------
    ...
    if pressured:
        ...
        if transition["before_threats"]:

            threat = ", ".join(
                transition["before_threats"][:2]
            )

            return (
                f"Avant ton coup, l'adversaire avait déjà une contrainte : "
                f"{threat}. Tu as joué {san_played}, mais ce coup ne "
                f"résout pas suffisamment cette contrainte. Après ton coup, "
                f"{target} reste sous pression. "
                f"Le problème est donc la réponse apportée à la position, "
                "pas simplement le fait que la pièce soit attaquée."
            )

        return (
            f"Ton coup {san_played} laisse {target} sous une pression "
            "supérieure à sa défense. La pièce n'est pas forcément en "
            "prise immédiatement, mais ton coup permet à l'adversaire "
    ...
    if transition["move_attacks"]:
        ...
        if best_reply_san and best_reply_san != "-":
            ...
            reply_text = (
                f" Le moteur donne {best_reply_san} comme meilleure "
                "réponse adverse ; il faut donc vérifier si cette "
                "contre-menace compense réellement ton problème initial."
            )

        if transition["before_threats"]:

            return (
                f"Avant ton coup, l'adversaire avait une menace : "
                f"{', '.join(transition['before_threats'][:2])}. "
                f"Tu as joué {san_played}, qui crée à son tour une "
                f"attaque sur {target_text}.{reply_text} "
                "Le point à comprendre est donc le rapport entre les "
                "deux menaces : une contre-menace n'est utile que si "
                "elle oblige réellement l'adversaire à abandonner "
                "ou à modifier son idée."
            )

    # --------------------------------------------------------
    # MENACE ADVERSE NON RÉSOLUE
    # --------------------------------------------------------

    if transition["before_threats"]:

        threats = ", ".join(
            transition["before_threats"][:3]
        )
        ...
        if best_reply_san and best_reply_san != "-":
            ...
            reply_text = (
                f" Après {san_played}, l'adversaire dispose de "
                f"{best_reply_san} comme meilleure réponse selon le moteur."
            )

        return (
            f"Avant ton coup {san_played}, l'adversaire menaçait "
            f"{threats}. Ton coup ne répond pas suffisamment à cette "
            "contrainte et ne crée pas de compensation concrète "
            "suffisante. C'est cette interaction entre la menace "
            "adverse et ton choix qui explique la perte d'évaluation."
            + reply_text
        )
    ...
    if phase == "Ouverture":
        ...
        undeveloped = context["user_development"]["undeveloped_minor"]
        ...
        if (
            ...
            return (
                f"Tu as joué {san_played}. Le problème ne vient pas "
                "du coup de pion en lui-même : après comparaison avec "
                f"la position créée par {san_played}, le développement "
                "des pièces reste le facteur prioritaire. Un coup qui "
                "développe directement une pièce aurait davantage "
                "amélioré ta position."
            )

    # --------------------------------------------------------
    # EXPLICATION GÉNÉRALE MAIS RELIÉE AU COUP
    # --------------------------------------------------------
    ...
    if best_reply_san and best_reply_san != "-":
        ...
        reply_text = (
            f" Après ton coup, la meilleure réponse adverse indiquée "
            f"par le moteur est {best_reply_san}."
        )

    return (
        f"Tu as joué {san_played} avec le {piece_text}. "
        "Le problème ne doit pas être résumé par « Stockfish préfère "
        f"{best_move_san} ». Il vient de la transformation de la position : "
        "ton coup ne traite pas assez efficacement les contraintes "
        "existantes ou permet à l'adversaire d'améliorer sa position "
        "avec un tempo."
        + reply_text
    )
