...
def analyze_game_with_engine(
    ...
    def accuracy_from_loss(loss_cp, eval_before_cp):
        try:
            before = win_percent_from_cp(eval_before_cp)
            after = win_percent_from_cp(
                eval_before_cp - max(0.0, loss_cp)
            )
...
def display_coaching_report(
    ...
    best_line = move_data.get("best_line", "") if move_data else ""
    if best_line:
...
def build_move_coaching(board_before, board_after, move, move_data, is_user_white):
    ...
    best_line = move_data.get("best_line", "")
    best_reply = move_data.get("best_reply", "-")
    loss = float(move_data.get("loss", 0.0))

    report["opponent_threats"] = _v3_concise_threats(board_before, is_user_white)
    report["problem"] = _v3_problem(
        board_before, board_after, move, best_san, loss,
        is_user_white, best_reply, best_line
    )
    report["consequences"] = _v3_consequences(
        board_before, board_after, move, is_user_white
    )
    report["best_explanation"] = _v3_best_explanation(
        board_before, best_san, loss, best_line
    )
    report["best_line"] = best_line
...
def display_coaching_report(report):
    ...
    st.success(best)
    st.write(report.get("best_explanation", "Explication indisponible."))
